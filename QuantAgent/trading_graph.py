...
# Qwen provider label is kept for compatibility, but routed through Xiaomi MiMo.
...
class TradingGraph:
    ...
    def _get_qwen_base_url(self) -> str:
        return str(self.config.get("qwen_base_url", "") or "").strip() or "https://token-plan-cn.xiaomimimo.com/v1"
    ...
    def _get_api_key(self, provider: str = "openai") -> str:
        ...
        if provider == "openai":
            ...
            # actual calls through Xiaomi MiMo.
            ...
            if api_key == "":
                raise ValueError(
                    ...
                    "You can get one from: https://token-plan-cn.xiaomimimo.com/v1"
                )
        else:
